...
require_once("dbtools.inc.php");
  ...
  //获取当前用户的等级
	...
	for ($i=0; $i <$total_num ; $i++) { 
		...
		echo "<form method='post' action='sql_user_return_book.php?&book_no=".$row["book_no"]."&book_name=".urlencode($row["book_name"])."&borrow_date=".$row["borrow_date"]."&return_deadline=".$row["return_deadline"]."&user_account=".$row["user_account"]."'>";
		...
		echo "<td><input type='submit' value='归还'></td>";
		echo "</tr>";
		echo "</form>";
	}
 ?>
